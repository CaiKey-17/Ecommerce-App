...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  final TextEditingController _newVersionImportPriceController = TextEditingController();
  final TextEditingController _newVersionOriginalPriceController = TextEditingController();
  final TextEditingController _newVersionQuantityController = TextEditingController();
  final TextEditingController _newVersionDiscountPercentController = TextEditingController();
  ...
  final TextEditingController _variantImportPriceController = TextEditingController();
  final TextEditingController _variantOriginalPriceController = TextEditingController();
  final TextEditingController _variantQuantityController = TextEditingController();
  final TextEditingController _variantDiscountPercentController = TextEditingController();
  ...
  final TextEditingController _colorQuantityController = TextEditingController();
  ...
  void initState() {
    ...
    if (widget.isEdit && widget.productInfo != null) {
      ...
      _descriptionController = TextEditingController(text: widget.productInfo!.shortDescription);
  ...
  void _updateInitialValues() {
    _initialValues = {
      ...
      'specifications': List<Map<String, String>>.from(specifications.map((spec) => Map<String, String>.from(spec))),
      ...
      'variantColors': variantColors
          .map((colors) => colors
              ...
              .toList())
          .toList(),
      'colorImages': colorImages.map((images) => List<String?>.from(images)).toList(),
      'hasColors': hasColors,
    };
  }
  ...
  void _checkForChanges() {
    ...
    if (_descriptionController.text != _initialValues['description']) hasChanges = true;
    ...
    if (_additionalImages.length != (_initialValues['additionalImages'] as List).length ||
        _additionalImages.asMap().entries.any((e) => e.value != (_initialValues['additionalImages'] as List)[e.key])) {
      hasChanges = true;
    }

    if (specifications.length != (_initialValues['specifications'] as List).length ||
        specifications.asMap().entries.any((entry) {
          int i = entry.key;
          Map<String, String> spec = entry.value;
          Map<String, String> initialSpec = (_initialValues['specifications'] as List)[i];
          return spec.keys.first != initialSpec.keys.first || spec.values.first != initialSpec.values.first;
        })) {
      hasChanges = true;
    }
    ...
    if (variants.length != (_initialValues['variants'] as List).length ||
        variants.asMap().entries.any((entry) {
          ...
          Map<String, dynamic> initialV = (_initialValues['variants'] as List)[i];
    ...
    if (variantColors.length != (_initialValues['variantColors'] as List).length ||
        variantColors.asMap().entries.any((entry) {
          int i = entry.key;
          List<ProductColor> colors = entry.value;
          List<Map<String, dynamic>> initialColors = (_initialValues['variantColors'] as List)[i];
    ...
    if (colorImages.length != (_initialValues['colorImages'] as List).length ||
        colorImages.asMap().entries.any((entry) {
          ...
          List<String?> initialImages = (_initialValues['colorImages'] as List)[i];
          if (images.length != initialImages.length) return true;
          return images.asMap().entries.any((imgEntry) => imgEntry.value != initialImages[imgEntry.key]);
        })) {
      hasChanges = true;
    }
  ...
  Future<void> _fetchBrandsAndCategories() async {
    ...
    try {
      ...
      final results = await Future.wait([brandNamesFuture, categoryNamesFuture]);
      if (!mounted) return;
      setState(() {
        brands = results[0] as List<String>;
        productTypes = results[1] as List<String>;
        if (brands.isNotEmpty && (selectedBrand == null || !brands.contains(selectedBrand))) {
          selectedBrand = brands.first;
        } else if (brands.isEmpty) {
          selectedBrand = null;
          errorMessage = "Không có danh sách hãng. Vui lòng kiểm tra API /admin/brand/names.";
        }
        if (productTypes.isNotEmpty && (selectedProductType == null || !productTypes.contains(selectedProductType))) {
  ...
  Future<void> _fetchVariants() async {
    if (!widget.isEdit || widget.productInfo == null || widget.productInfo!.id == null) {
      if (!mounted) return;
      setState(() {
        errorMessage = "Thông tin sản phẩm không hợp lệ. Vui lòng kiểm tra ID sản phẩm.";
      });
      return;
    }
    if (!mounted) return;
    ...
    try {
      ...
      final variantsData = await apiAdminService.getVariantsByProductId(productId);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _fetchColorsForVariants() async {
    for (int i = 0; i < variants.length; i++) {
      try {
        ...
        developer.log("Error fetching colors for variant ${variants[i].id}: $e");
        if (!mounted) return;
        setState(() {
          errorMessage = "Không thể tải danh sách màu cho biến thể ${variants[i].nameVariant}: $e";
        });
      }
    }
  }
  ...
  void _updateVariantControllers() {
    if (!mounted) return;
    if (selectedVersionIndex >= 0 && selectedVersionIndex < variants.length && variants.isNotEmpty) {
      final variant = variants[selectedVersionIndex];
      _variantImportPriceController.text = variant.importPrice?.toStringAsFixed(0) ?? '';
      _variantOriginalPriceController.text = variant.originalPrice?.toStringAsFixed(0) ?? '';
      _variantQuantityController.text = variant.quantity?.toString() ?? '';
      _variantDiscountPercentController.text = variant.discountPercent?.toString() ?? '';
      final calculatedPrice = _calculateFinalPrice(variant.originalPrice, variant.discountPercent);
  ...
  void _updateVariant() {
    ...
    if (selectedVersionIndex >= 0 && selectedVersionIndex < variants.length) {
      final originalPrice = double.tryParse(_variantOriginalPriceController.text) ?? variants[selectedVersionIndex].originalPrice ?? 0;
      int discountPercent = int.tryParse(_variantDiscountPercentController.text) ?? variants[selectedVersionIndex].discountPercent ?? 0;

      // Giới hạn discountPercent tối đa 50
      ...
      final calculatedPrice = _calculateFinalPrice(originalPrice, discountPercent);
      ...
      setState(() {
        variants[selectedVersionIndex] = ProductVariant(
          ...
          importPrice: double.tryParse(_variantImportPriceController.text) ?? variants[selectedVersionIndex].importPrice ?? 0,
          originalPrice: originalPrice,
          quantity: int.tryParse(_variantQuantityController.text) ?? variants[selectedVersionIndex].quantity ?? 0,
    ...
    if (selectedVersionIndex >= 0 &&
      ...
      setState(() {
        variantColors[selectedVersionIndex][selectedColorIndex] = ProductColor(
          ...
          colorName: _colorNameController.text.isNotEmpty
              ? _colorNameController.text
              : variantColors[selectedVersionIndex][selectedColorIndex].colorName,
          colorPrice: double.tryParse(_colorPriceController.text) ??
              variantColors[selectedVersionIndex][selectedColorIndex].colorPrice,
          quantity: int.tryParse(_colorQuantityController.text) ??
          ...
          fkVariantProduct: variantColors[selectedVersionIndex][selectedColorIndex].fkVariantProduct,
        );
        _checkForChanges();
      });
    }
  ...
  Future<void> _saveProduct() async {
    ...
    try {
      if (_nameController.text.isEmpty || selectedProductType == null || selectedBrand == null) {
      ...
      bool determinedHasColors = variantColors.any((colors) => colors.isNotEmpty);
      hasColors = determinedHasColors;

      ProductInfo productInfo = ProductInfo(
        id: _savedProduct?.id ?? (widget.isEdit ? widget.productInfo!.id : null),
        name: _nameController.text,
      ...
      if (_savedProduct == null && !widget.isEdit) {
        ...
        _savedProduct = await apiAdminService.updateProduct(productInfo.id!, productInfo);
        developer.log("Updated product with ID: ${_savedProduct!.id}");
      }

      if (_additionalImages.isNotEmpty || widget.isEdit) {
        if (widget.isEdit) {
          List<String> initialImages = List<String>.from(_initialValues['additionalImages']);
          List<String> currentImages = _additionalImages;

          List<String> imagesToAdd = currentImages.where((img) => !initialImages.contains(img)).toList();

          List<ProductImage> allImages = await apiAdminService.getImagesByProduct(_savedProduct!.id!);
          List<ProductImage> imagesToDelete = allImages.where((img) => !currentImages.contains(img.image)).toList();
          ...
          for (ProductImage image in imagesToDelete) {
            ...
            try {
              if (image.id != null) {
                ...
                developer.log("Deleted image with ID: ${image.id} (URL: ${image.image})");
              }
            } catch (e) {
          ...
          for (String imageUrl in imagesToAdd) {
            if (!mounted) return;
            ProductImage image = ProductImage(id: null, image: imageUrl, fkImageProduct: _savedProduct!.id!);
      ...
      for (int i = 0; i < variants.length; i++) {
        ...
        if (variantToSave.importPrice == null || variantToSave.importPrice! < 0) {
          throw Exception("Giá nhập không hợp lệ.");
        }
        if (variantToSave.originalPrice == null || variantToSave.originalPrice! < 0) {
        ...
        if (variantToSave.discountPercent == null || variantToSave.discountPercent! < 0 || variantToSave.discountPercent! > 100) {
          throw Exception("Phần trăm giảm giá không hợp lệ.");
        }
        if (variantToSave.discountPercent == null || variantToSave.discountPercent! < 0 || variantToSave.discountPercent! > 50) {
        ...
        if (variantToSave.id == 0) {
          variantToSave = ProductVariant(
            ...
            price: _calculateFinalPrice(variantToSave.originalPrice, variantToSave.discountPercent),
            fkVariantProduct: _savedProduct!.id!,
          );
          var savedVariant = await apiAdminService.createProductVariant(variantToSave);
          updatedVariants.add(savedVariant);
        } else {
          var updatedVariant = await apiAdminService.updateProductVariant(variantToSave.id, variantToSave);
      ...
      if (hasColors) {
        for (int i = 0; i < variants.length; i++) {
          for (int j = 0; j < variantColors[i].length; j++) {
            ...
            if (color.id == 0) {
              var savedColor = await apiAdminService.createProductColor(colorToSave);
              updatedVariantColors[i].add(savedColor);
              updatedColorImages[i].add(savedColor.image);
            } else {
              var updatedColor = await apiAdminService.updateProductColor(color.id, colorToSave);
      ...
      final product = {
        "id": _savedProduct!.id ?? "#00${DateTime.now().millisecondsSinceEpoch}",
        "name": _savedProduct!.name,
        "price": variants.isNotEmpty
            ? (hasColors && selectedVersionIndex >= 0 && selectedColorIndex >= 0 && variantColors[selectedVersionIndex].isNotEmpty
                ? variantColors[selectedVersionIndex][selectedColorIndex].colorPrice
                : variants[0].price)
            : 0,
        "quantity": variants.isNotEmpty
            ? (hasColors && selectedVersionIndex >= 0 && selectedColorIndex >= 0 && variantColors[selectedVersionIndex].isNotEmpty
                ? variantColors[selectedVersionIndex][selectedColorIndex].quantity
                : variants[0].quantity)
            : 0,
        ...
        "versionColors": variantColors.map((colors) => colors.map((c) => c.colorName).toList()).toList(),
        "versionColorPrices": variantColors.map((colors) => colors.map((c) => c.colorPrice).toList()).toList(),
        "versionColorQuantities": variantColors.map((colors) => colors.map((c) => c.quantity).toList()).toList(),
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(widget.isEdit ? "Cập nhật sản phẩm thành công" : "Thêm sản phẩm thành công")),
        );
    ...
    if (widget.isEdit && widget.productInfo?.id != null) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text("Bạn có chắc chắn muốn xóa sản phẩm này? Tất cả biến thể và màu sắc liên quan sẽ bị xóa."),
          actions: [
            TextButton(
  ...
  Future<void> _deleteProductAndVariants() async {
    if (_savedProduct == null || _savedProduct!.id == null) {
      ...
      throw Exception("Không có sản phẩm để xóa hoặc ID sản phẩm không hợp lệ.");
    }
    try {
  ...
  void _showAddColorDialog() {
    ...
    if (variants[selectedVersionIndex].id == 0) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Vui lòng lưu sản phẩm và phiên bản trước khi thêm màu.")),
        );
      }
      return;
    }
      ...
      builder: (dialogContext) {
        return StatefulBuilder(
          builder: (dialogContext, setDialogState) {
            return AlertDialog(
              title: Text("Thêm màu cho ${variants[selectedVersionIndex].nameVariant}"),
                    ...
                    if (_newColorController.text.isEmpty ||
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(dialogContext).showSnackBar(
                          SnackBar(content: Text("Vui lòng nhập đầy đủ thông tin màu.")),
                        );
                      }
                      return;
                    }
  ...
  void _showColorChoiceDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Bạn có muốn thêm màu sắc cho các phiên bản của sản phẩm này không?"),
        actions: [
          TextButton(
            onPressed: () {
              if (mounted) {
  ...
  void _showAddVersionDialog() {
    ...
    void _updateFinalPrice() {
      final originalPrice = double.tryParse(_newVersionOriginalPriceController.text) ?? 0;
      int discountPercent = int.tryParse(_newVersionDiscountPercentController.text) ?? 0;
                ...
                TextButton(
                  onPressed: () async {
                    if (_newVersionController.text.isEmpty ||
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text("Vui lòng nhập đầy đủ thông tin phiên bản.")),
                        );
                      }
                      return;
                    }
                    final importPrice = double.tryParse(_newVersionImportPriceController.text);
                    final originalPrice = double.tryParse(_newVersionOriginalPriceController.text);
                    final quantity = int.tryParse(_newVersionQuantityController.text);
                    int discountPercent = int.tryParse(_newVersionDiscountPercentController.text) ?? 0;
                    ...
                    if (discountPercent < 0) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text("Phần trăm giảm giá không hợp lệ.")),
                        );
                      }
                      return;
                    }
                    if (_nameController.text.isEmpty || selectedProductType == null || selectedBrand == null) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text("Vui lòng nhập đầy đủ thông tin sản phẩm trước khi thêm phiên bản.")),
                        );
                      }
                      return;
                    }
                    ...
                    try {
                      if (_savedProduct == null) {
                        ProductInfo productInfo = ProductInfo(
                          ...
                          mainImage: _imagePath,
                          shortDescription: _descriptionController.text,
                          detail: specifications.map((spec) => "${spec.keys.first}:${spec.values.first}").join(';'),
                          hasColor: false,
                        );

                        _savedProduct = widget.isEdit
                            ? await apiAdminService.updateProduct(productInfo.id!, productInfo)
                            : await apiAdminService.createProduct(productInfo);
                        developer.log("Saved product with ID: ${_savedProduct!.id}");
                      }
                      ...
                      developer.log("Saving variant: ${variantToSave.toJson()}");
                      ProductVariant savedVariant = await apiAdminService.createProductVariant(variantToSave);
                      ...
                      if (mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text("Đã thêm phiên bản thành công.")),
                        );
                      ...
                      if (mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text("Không thể lưu phiên bản: $e")),
                        );
                      }
                    } finally {
                      if (mounted) {
    ...
    if (variants[index].id != 0) {
      showDialog(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Text("Bạn có chắc chắn muốn xóa phiên bản '${variants[index].nameVariant}'? Tất cả màu sắc liên quan sẽ bị xóa."),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(dialogContext),
              child: Text("Hủy"),
            ...
            TextButton(
              onPressed: () async {
                ...
                try {
                  ...
                  developer.log("Deleting variant with ID: ${variants[index].id}");
                  await apiAdminService.deleteProductVariant(variants[index].id);
                  if (mounted) {
                    setState(() {
                      ...
                      if (variants.isEmpty) {
                        ...
                        selectedVersionIndex = variants.length - 1;
                        selectedColorIndex = variantColors[selectedVersionIndex].isEmpty ? -1 : 0;
                      }
                      _updateVariantControllers();
                      _checkForChanges();
                    });
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text("Đã xóa phiên bản thành công")),
                    );
                    Navigator.pop(dialogContext);
                  }
                } catch (e) {
                  ...
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text("Không thể xóa phiên bản: $e")),
                    );
                  }
                } finally {
                  if (mounted) {
  ...
  void _deleteColor(int index) {
    if (selectedVersionIndex < 0 || !mounted) return;
    if (variantColors[selectedVersionIndex].isEmpty || index >= variantColors[selectedVersionIndex].length) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Không tìm thấy màu để xóa.")),
        );
      }
      return;
    }
    if (variantColors[selectedVersionIndex][index].id != 0) {
      showDialog(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Text("Bạn có chắc chắn muốn xóa màu '${variantColors[selectedVersionIndex][index].colorName}'?"),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(dialogContext),
              child: Text("Hủy"),
            ...
            TextButton(
              onPressed: () async {
                ...
                try {
                  developer.log("Deleting color with ID: ${variantColors[selectedVersionIndex][index].id}");
                  await apiAdminService.deleteProductColor(variantColors[selectedVersionIndex][index].id);
                  if (mounted) {
                    setState(() {
                      ...
                      if (variantColors[selectedVersionIndex].isEmpty) {
                        selectedColorIndex = -1;
                      } else if (selectedColorIndex >= variantColors[selectedVersionIndex].length) {
    ...
    if (!widget.isEdit && _savedProduct != null) {
      bool? shouldCancel = await showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text("Bạn đang thêm sản phẩm. Bạn có muốn hủy và xóa sản phẩm cùng các phiên bản vừa tạo không?"),
          actions: [
            TextButton(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ProductDetailImagePicker(
                      ...
                      productId: widget.isEdit ? widget.productInfo?.id : null,
                    ),
                    SizedBox(height: 16),
                    TextField(
                    ...
                    productTypes.isEmpty
                        ? Text("Không có danh mục nào. Vui lòng kiểm tra API.")
                        : DropdownButtonFormField<String>(
                            value: productTypes.contains(selectedProductType) && selectedProductType != null
                                ? selectedProductType
                                : productTypes.first,
                            decoration: InputDecoration(
                    ...
                    brands.isEmpty
                        ...
                        : DropdownButtonFormField<String>(
                            value: brands.contains(selectedBrand) && selectedBrand != null ? selectedBrand : brands.first,
                            decoration: InputDecoration(
                              labelText: "Hãng",
                              border: OutlineInputBorder(),
                            ),
                    ...
                    variants.isEmpty
                        ? Column(
                            ...
                            children: [
                              Text(
                                widget.isEdit ? "Không tìm thấy phiên bản nào" : "Chưa có phiên bản nào",
                                style: TextStyle(color: widget.isEdit ? Colors.red : Colors.grey),
                              ),
                              if (widget.isEdit)
                                TextButton(
                                  ...
                                  GestureDetector(
                                    onTap: () {
                                      ...
                                      setState(() {
                                        ...
                                        selectedColorIndex = hasColors && variantColors[index].isNotEmpty ? 0 : -1;
                                        _updateVariantControllers();
                                        _updateColorControllers();
                                      });
                                    },
                                    child: Container(
                                      ...
                                      child: Column(
                                        children: [
                                          Text(
                                            variants[index].nameVariant ?? 'N/A',
                    ...
                    if (hasColors && selectedVersionIndex >= 0 && selectedVersionIndex < variants.length && selectedVersionIndex < variantColors.length) ...[
                      SizedBox(height: 16),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                      ...
                      variantColors[selectedVersionIndex].isEmpty
                          ...
                          : Wrap(
                              spacing: 10,
                              runSpacing: 10,
                              children: List.generate(variantColors[selectedVersionIndex].length, (index) {
                                return Stack(
                                  children: [
                                    GestureDetector(
                                      onTap: () {
                                      ...
                                      child: IntrinsicWidth(
                                        child: Container(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              Text(
                                                variantColors[selectedVersionIndex][index].colorName,
                                                style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold),
                                              ),
                                              Text(
                      ...
                      if (selectedColorIndex >= 0 &&
                        ...
                        Container(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              if (variantColors[selectedVersionIndex][selectedColorIndex].image != null)
                                Center(
                                  child: Image.network(
                                    variantColors[selectedVersionIndex][selectedColorIndex].image!,
                                    height: 100,
                                    width: 100,
                                    fit: BoxFit.cover,
                                  ),
