...
class _OrderScreenState extends State<OrderScreen> {
  ...
  Future<void> _fetchOrdersAndBills() async {
    ...
    try {
      ...
      final fetchedOrders = widget.couponId != null
      ...
      debugPrint('Đã tải ${orders.length} đơn hàng${widget.couponId != null ? ' với fkCouponId: ${widget.couponId}' : ''}:');
      for (var order in orders) {
        debugPrint('Order ID: ${order.id}, CreatedAt: ${order.createdAt}, fkCouponId: ${order.fkCouponId}');
      }
      ...
      for (var order in orders) {
        if (order.id != null) {
          ...
          if (order.idFkCustomer != null) {
            try {
              debugPrint('Tải thông tin khách hàng cho idFkCustomer: ${order.idFkCustomer}');
              final customer = await apiService.getUserById(order.idFkCustomer!);
              debugPrint('Khách hàng cho idFkCustomer ${order.idFkCustomer}: ${customer.fullName}');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.couponId != null ? "Đơn hàng sử dụng mã coupon" : "Quản lý đơn hàng")),
